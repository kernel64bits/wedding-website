...
//
// A small async helper used by both panels. The panel passes its local
// setSaveStatus so error/saved feedback stays scoped to that panel; the
// parent's onSuccess closes the sheet after a brief "Saved ✓" pause.
...
// ── Edit panel ───────────────────────────────────────────────────────────────
//
// Used with `key={selectedInv.id}` from the parent so it remounts on selection
// change. That makes the useState initializer below run again with fresh
// props, eliminating the stale-data problem the previous useEffect had to
// paper over (T11.5).
function EditPanel({
  selectedInv,
  baseUrl,
  ...
  selectedInv: InvitationWithAttendees;
  baseUrl: string;
  onClose: () => void;
}) {
  const router = useRouter();
  const [editTableNumber, setEditTableNumber] = useState(
    selectedInv.tableNumber?.toString() ?? "",
  );
  ...
  function handleSaveEdit() {
    submit(
      `/api/admin/invitations/${selectedInv.id}`,
      {
        method: "PATCH",
  ...
  async function handleDelete() {
    try {
      await fetch(`/api/admin/invitations/${selectedInv.id}`, {
        method: "DELETE",
      });
      router.refresh();
      onClose();
              ...
              >
                ...
                <span className="text-muted-foreground">
                  {a.attending === true
                    ...
                    ? "✗"
                    : "—"}
                  {a.dietaryRestrictions ? ` · ${a.dietaryRestrictions}` : ""}
                </span>
              </div>
            ))}
          </div>
...
// ── Create panel ─────────────────────────────────────────────────────────────
//
// Mounted only while selectedId === "new". Same key-based remount strategy
// keeps state fresh; closing the sheet unmounts it and clears the form.
              ...
              <div key={i} className="flex items-center gap-2">
                ...
                {newAttendees.length > 1 && (
                  <button
                    type="button"
                    onClick={() =>
                      setNewAttendees(newAttendees.filter((_, j) => j !== i))
                    }
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        >
          {selectedId === "new" ? (
            <CreatePanel key="new" onClose={() => setSelectedId(null)} />
          ) : selectedInv ? (
            <EditPanel
              key={selectedInv.id}
